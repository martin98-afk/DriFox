...
- **renderer 内存看门狗：对话变长全白屏的程序化自救** (`app/widgets/renderer_watchdog.py` 新增, `app/main_widget.py`, `app/widgets/webview_pool.py`, `tests/widgets/test_renderer_watchdog.py` 新增, `tests/widgets/test_webview_pool.py`): 长对话 + 大 base64 图（单会话 20+ 张截图）场景下，承载可见卡片的 renderer 进程（Chromium 按 site 复用）内存单调涨到 3.17GB（2026-09-15 22:41 取证）后 V8 窒息 → 其上全部视图白屏、新建对话也落同一进程 → 全空白；此时进程未崩溃，`renderProcessTerminated` 不触发，`message_card` 三重自愈链全程沉睡，只能重启应用。修复：新增后台线程看门狗（60s 采样本进程 `--type=renderer` 子进程 per-pid RSS，同一 PID 连续 3 周期超 1280MB 判定病态，全局冷却 600s，`DRIFOX_RENDERER_WATCHDOG=0` 可停用）→ 宿主剔除池中该 PID viewer（`WebViewPool.purge_pid`）+ terminate 进程 → Chromium 向其上全部视图广播 `renderProcessTerminated`，既有自愈链（重载骨架 → 补渲 → needRecreate）接管恢复，新视图落新进程内存清零。与 B4 强回收层互补：B4 只 kill 离屏已卸载 renderer，本模块覆盖「在用 renderer 病态」缺口。环境开关 `DRIFOX_RENDERER_SICK_MB` 可调阈值。

...
- **滚到底还能继续下滚出大片空白、停止滚动后消失** (`app/main_widget.py`, `app/widgets/height_commit_batch.py`): 卡片高度由 WebEngine JS 异步上报（80ms 防抖 + 平滑限幅分帧 + 流式收拢 <40px 抑制），流式期间某卡 `fixedHeight` 常暂时大于内容实际高度；`_sync_scroll_maximum` / `HeightCommitBatch.flush` 原本「只抬高不压低」的校正把这个虚高抬进滚动条上界，贴底钉底路径 `setValue(maximum)` 随即把视口送进内容底之下的越界区；虚高收回（下一次上报 / 布局重算）后 maximum 收缩、value 被钳回，即「停止滚动空白又消失」。修复：贴底（value 距上界 ≤ AT_BOTTOM_TOLERANCE）时把上界双向校正到 `sizeHint` 真实值（`_sync_scroll_maximum` 新增压低分支 + flush follow 分支改无条件校正），非贴底保持只抬高不动（懒渲染重建窗口 sizeHint 短暂塌陷、等高占位防的就是它）；`_maintain_bottom_anchor` 钉底前补一次校正。
...
- **插件页 × 与 diff 回退静默失效（PySide6 拒绝非 QObject 槽的 UniqueConnection 连接）** (`app/plugins/registries/ui_plugin_registry.py`, `app/widgets/cards/global_card_controller.py`): `connect(self._xxx, type=Qt.UniqueConnection)` 中槽的宿主 `UIPluginRegistry` / `GlobalCardController` 均为普通类（非 `QObject` 子类），PySide6 直接拒绝这类连接，仅往 stderr 打一行 `qt.core.qobject.connect` 警告、**不抛 Python 异常** → 信号照常 emit 却无人接收，表现为「点关闭钮没反应且日志零报错」（右侧工作台插件页 tab ×、关掉 diff 查看器不回到文件撤销视图）。修复：去掉 `UniqueConnection`，去重幂等标志从 registry 单例改挂 panel / diff 卡实例（实例重建即复位，不留「以为连过」的谎）；`_close_workbench_card_tab` 原 `except Exception: pass` 改为留痕告警——静默吞异常正是本次难定位的直接原因。全仓同类写法已清点：`chart_viewer_card` / `llm_settings_card` 宿主为 `QFrame` 子类，合法保留。新增 6 条回归用例；关键教训——**`_FakeSignal` 桩的 connect 无条件收下 slot，结构上测不出本断点**，真信号链用例必须用真 `QObject` + 真 `Signal` + 真 `QPushButton.click()`。
- **ThreadGuardWatchdog 抛 RuntimeError: QThread already deleted** (`app/utils/thread_guard.py`): `_running_threads` 的强引用只防 Python GC，挡不住主线程把 C++ 对象析构；`isinstance` 只校验 Python 包装器类型、不校验 shiboken 指针，纯 Python 看门狗线程调 `isRunning()` 必踩空（snapshot 之后的竞态，或销毁未触发 `destroyed` 致条目永久滞留、每 30s 稳定抛错）。修复：扫描前 `shiboken6.isValid()` 探活 + `isRunning()` 外层兜 `RuntimeError`，失效条目就地剔除；扫描体抽成 `_scan_stuck_threads()` 以便单测直接驱动。
...
- **CI 双轨发布流程** (`.github/workflows/release.yml`): dev/pyside6 共用 tag，产物加 `-qt5` / `-pyside6` 后缀，避免 PySide6 与 PySide6 分支产物互盖。
...
- **会话切换 sentinel 保护**: 实现会话切换哨兵机制，防止会话间消息内容互相覆盖
- **视觉注入文本包裹优化**: 将视觉注入文本包裹在 system-reminder 标签中，保持上下文一致性
- **图片体积自动压缩防止 API 400**: 为 `screenshot`/`read` 工具结果注入和用户图片附件添加 >5MB 自动压缩（基于 PySide6 QImage 等比缩小 + JPEG quality=85），避免 4K 截图因 base64 过大导致 API "media exceeds size limit" 错误，解决 PyInstaller 打包后因 PIL 缺失 PNG 略大而更容易触发的问题

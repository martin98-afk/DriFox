...
自上一版本以来的变更 | 提交数：1 · 文件变更：4 · +5/-5 | 贡献者：mading

### 🔧 其他 (Chores & Build)

- 升级版本号至 v0.5.11（`pyproject.toml`、`app/utils/config.py`、`dist/installer.iss`、`README.md`）
- 本版本代码内容与 v0.5.10（重新发布版）一致，无功能变更；发布目的是走一遍真实发布链路，验证 v0.5.10 引入的**应用内半静默更新**：在 v0.5.10 实例里点「立即更新」，应看到下载完成后只弹安装器进度条窗口（无向导页、零点击），装完自动以原用户身份拉起 v0.5.11，失败时 `%TEMP%\Drifox-Setup.log` 可查。
- 验证要点：① 升级前确认已安装的是 v0.5.10 **重新发布版**产物（14:53 之后那次构建），旧 v0.5.10 产物不含该逻辑；② 观察是否出现「装完没自动启动」或「旧进程未退导致文件占用报错」；③ 更新后 `设置 → 关于` 版本号应为 v0.5.11。

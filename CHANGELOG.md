...
- **权限审批卡重写为专属卡片** (`app/widgets/cards/floating/permission_approval_widget.py` 新增, `app/core/workers/chat_worker.py`, `app/core/engines/ui/engine.py`, `app/core/conversation/backend.py`, `app/main_widget.py`, `app/widgets/modules/system_cards_module.py`, `tests/widgets/test_permission_approval_widget.py` 新增, `tests/widgets/test_permission_decided_host.py` 新增, `tests/core/test_permission_decided_signal.py` 新增): 工具权限审批此前完全复用「提问卡片」，用户看到的是「下一步/提交」按钮、单选列表和自定义输入框，与实际语义（允许/拒绝一个危险操作）不符且信息展示不足。现新建专属审批卡：底部左区「预览」「记住」（RoundMenu 下拉，含本轮/会话两档），右区「拒绝」「允许」（允许恒在最右作主操作），卡片直接展示工具名、命令原文、影响范围（路径列表 + 是否写）与来源标签（「⚠ 安全中心拦截」/「🔒 工具权限策略」）；字号收敛为三档随窗口缩放，移除页码与数字键提示。**回传链路全面结构化**：此前靠「问题「…」的回答：\n【允许】」文本标签反解析（用户自定义输入含该字样即误判，且标签改字会导致静默 deny），现改为 `answered(decision, remember, reason)` 三参结构化信号，经 worker → engine → backend 三层传递，杜绝静默失效。**安全默认强化**：默认焦点落在「拒绝」、危险档位「允许」按钮延迟解锁（鼠标与键盘路径同受约束）、空提交/异常路径一律判拒绝。**三层防静默丢弃**：宿主自持 `tool_call_id`、门面返回 bool 明确成败、worker 在 id 不匹配时记 warning。同时修复三项缺陷：审批异常时输入区不恢复（永久隐藏）、子智能体权限请求无提示（静默卡住）、僵尸卡态（取消流后卡片残留遮挡新请求，提问卡同样受影响）。测试新增 92 例。

- **沙箱判定补强（含命令类工具的路径边界）** (`app/tools/sandbox.py`, `tests/tools/test_sandbox_command_path.py` 新增, `tests/tools/test_sandbox_path_arrays.py` 新增, `tests/tools/test_sandbox_path_relative_whitelist.py` 新增, `tests/tools/test_sandbox_explicit_workdir.py` 新增): **命令类工具此前完全跳过路径判定** —— `sandbox_check_tool` 的 command 分支只判命令名与网络，从不调 `check_path`，导致配置的 `path.blacklist` 对 `bash` 工具零作用（AI 用 `type D:/secrets/key.pem` 可静默读取敏感目录）。现补入**黑名单定向检查**：遍历命令 token（含引号内子命令二次拆分），对非 flag 项按 `read` 模式过 `check_path`（read 模式天然只查黑名单，规避 `rm`/`type`/`git` 等命令的读写模式静态推断难题），任一命中即转审批；黑名单为空（默认）时整块短路，零性能开销。同时修复：数组字段路径漏判（`files`/`paths`，任一子项命中即整体拦）、白名单相对条目按进程 cwd 而非 workdir 解析导致失效（含 `%USERPROFILE%\docs` 类环境变量条目先展开再判绝对）、黑名单前缀匹配过宽（`D:/secrets` 误命中 `D:/secrets2`）、删除豁免相对条目在绝对路径入参时失效、裸域名不触发域名黑名单（`curl evil.com/x` 现可识别）、`sandbox_check_tool` 新增显式 `workdir` 参数（多窗口/主子并发时消除全局 workdir 串味）。测试新增 36 例。

- **`git` 破坏性子命令纳入审批 + 命令前缀名单改为词边界匹配** (`app/tools/command_safety.py`, `app/tools/sandbox.py`, `tests/tools/test_git_subcommand_guard.py` 新增): ① `classify_command` 只看首 token，`git` 不在确认名单 → **所有 git 命令一律判 safe**，`git rm -rf .` / `git clean -fdx` / `git reset --hard` / `git push --force` 均不触发审批。现新增 `_git_subcommand_verdict()` 按**子命令 + 选项组合**判定（`git rm`/`filter-branch`/`update-ref` 命中即审批；`git clean` 需 `-f`、`git push` 需 `-f`/`--force`、`git branch` 需 `-D` 才判，避免误伤 `git clean -n` 与 `git branch --list`），并正确处理 `git -C <path>` / `-c k=v` 等全局选项前置形态。② 用户命令名单（放行/强制审批）此前是纯 `startswith` 前缀匹配 → 白名单填 `git` 会连带放行 `git rm -rf .`、`git push --force`；现改为**词边界对齐**（命令等于前缀，或前缀后紧跟空白），且**白名单命中后仍检查危险命令判定层**（填 `git` 可省常规子命令审批，但 `git rm -rf .` 仍会确认）。测试新增 47 例。

- **子智能体与 gateway 补齐沙箱判定（两条执行层旁路）** (`app/core/workers/subagent_worker.py`, `app/gateway/local_service/session_handler.py`, `tests/core/test_subagent_worker_sandbox.py` 新增, `tests/gateway/test_gateway_sandbox_permission.py` 新增): 审计发现两条**完全绕过沙箱**的执行路径。① **子智能体**：`_execute_tools` 只有工具开关一层门，无沙箱判定 → 子智能体可执行 `rm D:/重要文件`、外传命令而不触发审批。现前置接入 `sandbox_check_tool`（**显式传 `workdir`** 消除全局串味），`confirm`/`deny` 一律按拒绝处理（子智能体跨线程无 UI 交互能力，不弹窗），回填文案引导模型改走主对话。② **gateway**：`session_handler` 收到权限事件**无条件自动放行**（无人值守场景的刻意设计），但新增的 `gateway_auto_approve` 配置键此前**无任何消费点**（改了没效果）。现接入：默认 `True` 保持现状；设为 `False` 时遵循沙箱判定（`allow` 仍放行，`confirm`/`deny` 拒绝并回事件给平台）；读配置失败保持放行（fail-open），沙箱判定失败取保守拒绝方向。测试新增 19 例。

- **LSP 安全门禁修复 — LSP 功能首次真正可用** (`app/core/lsp/lsp_client.py`, `app/core/lsp/lsp_config.py`, `app/core/lsp/lsp_manager.py`, `app/core/services/plugin_host_service.py`, `app/plugins/managers/plugin_manager.py`, `app/widgets/cards/settings/lsp_setting_card.py`, `tests/core/test_lsp_gate_source.py` 新增): LSP 启动门禁的 `source` 参数决定「内置/非内置」判定，但 LSP 侧 4 处调用**全部未传** → `is_builtin_source(None)` 恒 False → 所有 LSP server（含系统插件提供的）一律判「非内置源首次启动」→ 需用户确认，**而 LSP 设置卡没有确认入口** → **任何 LSP 语言服务器都无法启动**，`lsp` / `get_diagnostics` 工具静默返回「启动被安全门禁拦截」。现补全 source 透传全链路（`LspServerConfig.source_path` 字段 + `.lsp.json` 实际路径 + 启动时传门禁），覆盖热重载（`add_plugin_servers`）与 CLI fallback（此前连 `plugin_name` 都传空串，属双重失效）路径；LSP 设置卡新增「放行」确认入口（对齐 MCP 卡范式）。测试新增 11 例。

- **hook 相对导入范围限制（链接逃逸防护）** (`app/core/hooks/hook_manager.py`, `tests/core/test_hook_relative_scope.py` 新增): Python 型 hook 的相对导入（`.module:func`）此前**无任何范围校验**，实测可通过 junction（`mklink /J`，无需管理员）或硬链接指向 hooks.json 同目录之外，导入任意模块。现加双重校验：① `resolve()` 后必须仍在 hooks.json 所在目录的子树内（拦 junction / 符号链接）；② 拒绝任何链接文件本身（`is_symlink()` 或 `st_nlink > 1`，拦硬链接 —— `resolve()` 无法识别硬链接，实测仅靠①会漏判）。**不套用标准路径白名单**（插件模块不可能位于 `app.hooks`/`app.utils`，套用会废掉全部插件 hook）；不改执行语义（hook 保持静默执行）。顺带修正 `tests/core/test_a2_hook_security.py` 的历史注释 —— 原注释称「相对路径保留旁路」，易被读成"存在已知未修的绕过漏洞"，实测 `..` 各种变体早已被 `replace(".", "/")` 意外封死。测试新增 17 例（含 junction / 硬链接真机验证）。

- **备份容量清理接入 + 快照分治** (`app/utils/file_operation_recorder.py`, `main.py`, `app/widgets/cards/settings/security_center_card.py`, `tests/utils/test_backup_limit_wiring.py` 新增): `enforce_backup_limit` 此前**零生产调用**（函数存在、有测试、无人调用），备份目录随使用无限增长；设置页「上限 3000 MB」仅是文案。现接入两处：启动期后台线程派发（万级文件实测 11.7s，不阻塞启动链）+ 设置页「立即清理」按钮。**分治配额**：FileRecorder 编辑备份（高频产物）与删除保护快照（误删后唯一恢复手段）此前共用同一 FIFO 配额，日常编辑量大的用户会把快照挤干净 —— 现拆为两个独立配额，快照 = 总配额 1/4（下限 100 MB），互不挤占。
...
- **托盘右键菜单改 Fluent 风格并补齐入口** (`app/tray_manager.py`, `app/widgets/modules/tray_menu.py` 新增): 原生 `QMenu` + emoji 菜单换成 qfluentwidgets `SystemTrayMenu`（圆角、跟随亮暗主题、Fluent 矢量图标），emoji 全部去除；菜单项改为「打开主界面 / 检查更新（右侧显示当前版本号）/ 打开日志 / 打开日志文件夹 / 重启 / 退出」。新增 `TrayContextMenu` 与 `HintMenuItemDelegate`：Qt 原生菜单项没有「右侧说明文本」概念，版本号走 action 动态属性 + 自定义委托绘制，并把提示宽度计入菜单项右侧留白（覆写 `_longestShortcutWidth`），否则会与菜单项文字重叠。菜单改为一次性构建，不再挂 `aboutToShow` 重建 —— `RoundMenu.clear()` 只清 action 与 subMenu，清不掉 `addSeparator()` 插入的裸 `QListWidgetItem`，反复重建会累积分隔线。日志路径与 `main.py` 传给 `setup_logging` 的 `get_app_data_dir()/logs` 一致：「打开日志」用系统默认程序打开全量 `all.log`，文件尚未落盘时退化为打开目录；检查更新前先拉起主界面，保证结果 InfoBar 有地方落地。`8cb6bcb2`

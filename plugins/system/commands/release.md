...
type: prompt
argument-hint:
  "<version>": "版本号（如 v0.2.10，可选，不传则自动自增一个小版本）"
...
**参数解析**：
- 第一个参数为 `<version>`（可选），格式如 `v0.2.10`
- 如果未传 `<version>`，自动从 `app/utils/config.py` 读取当前版本号并自增一个小版本（如 `v0.2.9` → `v0.2.10`）
- `--dry-run` 可选，仅生成 changelog 预览，不提交不推送

---

### 阶段 A：准备工作（验证环境 & 自动版本自增）
...
3. 如果没有传入版本号，自动读取当前版本并自增一个小版本
   → 从 app/utils/config.py 读取 current_version 值（如 "v0.2.9"）
   → 去掉 v 前缀，按 . 分割，最后一段数字 +1，重组为新版本号（如 "v0.2.10"）
   → 输出提示："ℹ 未指定版本号，自动自增: v0.2.9 → v0.2.10"
4. 确认新版本号格式正确（以 v 开头，如 v0.2.10）
   → 如果格式不对就报错退出
5. 确认该版本号尚未使用

...
| 按钮 | 操作 | 说明 |
|---|---|---|
| `+`（新建成员） | 弹出角色菜单 | 列出当前模板所有角色，**已加入团队的角色置灰**，点击未加入角色即为该角色新建成员会话 |
| 菜单底部「批量补齐全部角色」 | 一键创建 | 为模板中**所有未加入**的角色批量创建成员会话（已存在的跳过，不重复创建） |

行为约束：
- **补成员进入当前团队框**：新建成员会话复用团队现有 `run_id`（与当前团队同分组），不会新建团队框
- **无模板团队**（手动 `/team --join=<agent>` 加入的团队）：`+` 按钮点击提示「请先 /team --load=<模板>」；角色已全部加入时提示「团队角色已齐」
- 单个/批量新建均自动在 300ms 后完成成员 join 并网格排列窗口
<!-- end -->

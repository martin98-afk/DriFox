...
团队加载后，左侧 Tab 面板会出现团队分组框，其 header 提供快捷操作按钮（hover 显示）：

| 按钮 | 操作 | 说明 |
|---|---|---|
| `▶`（新建任务） | 一键开启新一轮任务 | 团队**所有成员窗口**内部新建会话（清空当前对话、保留窗口）+ 团队级生成**新 run_id**——新任务所有成员共享新 run_id，历史独立条目（旧任务归旧 run_id） |
| `+`（快速新建成员） | 弹出角色菜单 | 列出模板角色 ∪ 当前成员角色，**全部可点、可重复角色**（可建多个 build 等），选择后新建成员会话并入当前 run_id |

行为约束：
- **新建任务顺序保证**：先全员保存旧历史（旧 run_id）→ 再生成新 run_id → 更新所有成员窗口（后续会话保存落新 run）
- **无模板团队**（手动 `/team --join=<agent>` 加入的团队）：`+` 菜单仍可列出当前成员角色供重复创建；无模板且无成员时提示先加载模板或加入成员
- 快速新建成员/模板加载均自动在 300ms 后完成成员 join 并网格排列窗口
<!-- end -->

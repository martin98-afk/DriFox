...
## mermaid 语法铁律（2026-09-05 新增，模型与自审共用）

mermaid 10.x 解析严格：**一行一个独立语句**。下面五条每次写 mermaid 前过一遍，否则直接 Syntax error → 宿主 fallback 显示源码。

1. **节点定义独占一行**：`A[标签]:::class` 写完换行，再写箭头。
   反例：`A[标签]:::class    A --> B` —— 节点定义 + 箭头挤同一行，parser 报 Syntax error（黑实测过两轮，第一轮就栽这）。
2. **每个箭头独占一行**：不堆叠 `A --> B --> C` 在同一行（mermaid 接受但易引入歧义，分行更稳）。
3. **每个 classDef 独占一行**：换行作为语句结束符。
   反例：`classDef A fill:..    classDef B fill:..` —— 第二个 classDef 紧贴上一个，parser 拒绝。
4. **节点 ID 与箭头起点规则**：`A` 被 `A[label]` 定义后，`A --> X` 才合法；不能先 `[label]` 定义再同行紧跟 `A -->`（属第 1 条特例）。
5. **标签字符宽松**：`[]` / `{}` / `()` 内 `?` `→` 中文都合法，无需转义或包裹引号。但若 parse 报错，**先剥离标签内容到最小集（纯英文单词）验证**再回加。

**写完 checklist**：
- [ ] 每行只放一个节点定义 / 箭头 / classDef / subgraph 头 / end
- [ ] 没有「节点定义 + 箭头」挤同一行
- [ ] 没有「多个 classDef」挤同一行
- [ ] 节点 ID 没在不同行被重复定义成不同形状（`A[label]` 和 `A(label)` 冲突）

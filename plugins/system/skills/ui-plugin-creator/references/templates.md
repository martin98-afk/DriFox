...
- **一、浮动卡片模板（最常用）** — L54–L553
  - 1.1 完整骨架 — L56–L543
  - 1.2 按需裁切 — L544–L553
- **二、内容块渲染器模板** — L554–L631
  - 2.1 入口 — L556–L579
  - 2.2 渲染器函数 — L580–L631
- **三、消息元素工厂模板** — L632–L661
- **四、完整插件注册入口 + plugin.json** — L662–L746
  - 4.1 plugin.json — L664–L683
  - 4.2 ui/__init__.py 完整模板 — L684–L746
- **五、外部依赖管理（_vendor/ 模式）** — L747–L961
  - 5.1 适用场景 — L749–L756
  - 5.2 目录结构 — L757–L774
  - 5.3 复制依赖到 _vendor/ — L775–L794
  - 5.4 cards.py 中使用 _vendor/ 包 — L795–L821
  - 5.5 完整 register_ui 模板（含 _vendor/） — L822–L915
  - 5.5.1 常见陷阱：sys.modules 缓存 — L916–L955
  - 5.6 验证 _vendor/ 在 PyInstaller 打包后可用 — L956–L961
- **六、集成可复用 widgets 库** — L962–L1264
  - 6.1 在浮动卡片中嵌入统计卡片和图表 — L977–L1065
  - 6.2 主题色注入（拉模型 + chart_style 缓存） — L1066–L1080
  - 6.3 SQLite 异步读取模式 — L1081–L1150
  - 6.4 14 天窗口查询模板（最常用 SQL 模式） — L1151–L1185
  - 6.5 字段 fallback 模式（新字段缺失时回退估算） — L1186–L1218
  - 6.6 完整项目结构推荐 — L1219–L1246
  - 6.7 常见陷阱 — L1247–L1264
- **七、通用弹窗模板** — L1265–L1522
  - 7.1 确认弹窗类 — L1269–L1418
  - 7.2 快捷函数 — L1419–L1471
  - 7.3 在卡片中使用 — L1472–L1489
  - 7.4 补充：卡片 _apply_latest_theme 需缓存 theme_colors — L1490–L1511
  - 7.5 所需额外 import — L1512–L1522
- **八、欢迎卡片插件 tab（HTML/echarts 注入会话初始卡片）** — L1528–L1775
  - 8.1 适用场景 — L1537–L1548
  - 8.2 注册 API — L1549–L1566
  - 8.3 完整模板（calendar 参考实现） — L1567–L1642
  - 8.4 关键约束与技巧（踩坑记录） — L1643–L1701
    - 8.4.1 `<script>` 不执行 → 内容 Python 预渲染 + onclick 内联 JS — L1645–L1654
    - 8.4.2 onclick 属性引号冲突 → 用 DOM API + 占位符替换 — L1655–L1662
    - 8.4.3 `<style>` 注入后生效 → 样式全内联 — L1663–L1667
    - 8.4.4 明暗主题：用主程序注入的 `ctx["is_dark"]`，prefers-color-scheme 只做兜底 — L1668–L1695
    - 8.4.5 欢迎卡片 mode 持久化（无需插件处理） — L1696–L1701
  - 8.5 echarts 复杂图表渲染（参考实现：context-stats） — L1702–L1759
    - 8.5.1 渲染链路（主程序内部） — L1709–L1722
    - 8.5.2 render_func 返回 echarts 代码块 — L1723–L1737
    - 8.5.3 echarts JSON 的关键约束（坑） — L1738–L1749
    - 8.5.4 复杂图表能力（context-stats 已实现，可复用模式） — L1750–L1759
  - 8.6 验证清单 — L1760–L1775
- **九、输入框按钮模板（快捷动作类）** — L1786–L1925
  - 9.1 注册 API 与 position 锚定 — L1793
  - 9.2 ui/__init__.py 完整骨架 — L1817
  - 9.3 剪贴板写图规范（坑）setImage 非 setPixmap — L1862
  - 9.4 提示通道规范（坑）InfoBar 非 QToolTip — L1880
  - 9.5 临时全屏窗口生命周期模式 — L1900
  - 9.6 验证清单 — L1910
- **十、标题栏常驻 tab 模板（tab + full 卡组合）** — L1929–L2006
  - 10.1 组合模式：titlebar_tab + full 容器浮动卡 — L1936
  - 10.2 踩坑记录（来自 agent_trace 实战） — L1974
  - 10.3 验证清单 — L1994
- **十一、右侧工作台页模板（workbench_tab）** — L2010–L2064
  - 11.1 注册 — L2016
  - 11.2 页面 widget 约定（参考 _worktree_page.py） — L2036
  - 11.3 workbench_tab vs workspace_page（别选错扩展点） — L2046
  - 11.4 验证清单 — L2056
## 一、浮动卡片模板（最常用）
...
> 参考实现：`plugins/system/ui/__init__.py` + `_worktree_page.py`（工作树）、`_artifacts_page.py`（产物）。

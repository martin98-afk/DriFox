...
| P053 | 虚拟滚动滚回后消息卡片页脚行被撑成数百 px、模型胶囊拉伸成竖长条（畸变） | T42 高度守恒 `_apply_placeholder_height` 用 `setFixedHeight(H)` 钉死卡片（min=max），而 viewer 高度上报链（`_commit_viewer_height` / `HeightCommitBatch.flush`）只改 viewer 自身；H 与重建后内容实际高度（宽度 reflow / 折叠态 / 多卡均摊误差）的差被卡片布局压给仅有的两个可伸缩项（气泡容器 + 页脚栏），页脚模型胶囊带边框拉伸即成竖条 | 钉死必须带标记并在首个 viewer 真实高度到达时解除：`card.pin_layout_height(h)` 打标 + `_commit_viewer_height` 与 `HeightCommitBatch.flush` 双出口调 `_unpin_layout_height`；解除与 viewer 落高同一次布局收敛，无可见跳动。回归测试 `tests/widgets/test_t42_pin_unpin.py` |
| P035 | 简洁模式工具**完成框**偶现沉底（滞留工具区最底部，仅当工具完成后无新正文块） | **两层**：① restore 恒 appendChild 沉底 + append_tool_result 原地 replaceChild 转换（继承 data-order、物理位置不动）→ data-order 正确但物理在底；S1 后键集合恒同且无块缺 data-order → reorganizeContent 键序列 diff 全绿 + `_assignedDataOrder=False` → **跳过 sort → 沉底固化**。② **data-order 双尺度碰撞**：JS 注入块 = 锚点前 think/tool 计数，D+ 补齐块 = 容器 blocks 序号，跨尺度相等时 sort 稳定排序与单调性检查（严格小于）都不纠 → 物理底部滞留。且 append_tool_result 之后的渲染可能走差量快路径或不再有下一拍（S1 末轮）→ **任何依赖"下次 reorganizeContent"的修正都可能没有执行时机** | 完成块注入 JS 内**就地插位**（`_insertByOrder`，找第一个 data-order `>=` 的非运行中块 insertBefore——相等也插，语义为"工具在其调用位置后、后续思考前"），不依赖后续渲染；reorganizeContent 快路径保留 data-order 物理单调性检查作二道防线；改骨架 JS 必须递增 `_SKELETON_CACHE_VERSION`；复现/验证脚本在 `tests/debug/tool_completion_sink_repro.py`、`tests/debug/tool_insert_order_check.py` |

...
| P035 | 简洁模式工具**完成框**偶现沉底（滞留工具区最底部，仅当工具完成后无新正文块） | restore 恒 appendChild 沉底 + append_tool_result 原地 replaceChild 转换（继承 data-order、物理位置不动）→ data-order 正确但物理在底；S1 后键集合恒同且无块缺 data-order → reorganizeContent 键序列 diff 全绿 + `_assignedDataOrder=False` → **跳过 sort → 沉底固化** | `_orderChanged` 追加 **data-order 物理单调性检查**（跳过 `tool-streaming-block` 与无 data-order 块），物理倒序 → 强制 sort（getPos 是权威排序，误触发零视觉影响）；改骨架 JS 必须递增 `_SKELETON_CACHE_VERSION` |

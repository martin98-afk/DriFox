...
- **Phase 1：澄清技术细节** — L172–L219
  - 1.1 容器选择详解 — L188–L205
  - 1.2 异步 vs 同步 — L206–L219
- **Phase 2：创建插件目录结构** — L220–L274
  - 2.1 标准结构（系统插件 + 用户插件通用） — L222–L236
  - 2.2 最小结构（最简卡片） — L237–L248
  - 2.3 plugin.json 模板 — L249–L274
- **Phase 3：按模板生成代码** — L275–L307
  - 3.1 推荐顺序 — L277–L284
  - 3.2 设计约束（插件闭包） — L285–L291
  - 3.3 不需要从零写的部分 — L292–L307
- **Phase 4：验证** — L308–L350
  - 4.1 代码质量 — L310–L317
  - 4.2 实际加载测试 — L318–L329
  - 4.3 调试技巧 — L330–L344
  - 4.4 完整验证清单 — L345–L350
- **Phase 5：提交与发布** — L351–L389
  - 5.1 提交规范 — L353–L361
  - 5.2 用户插件发布 — L362–L380
  - 5.3 提交时附带的文档 — L381–L389
## ⚠️ 工作流总览（必读）
...
| 要**独立顶级窗口**吗？ | 是 → `register_window`（模板见 `templates-window.md`） | 窗口可脱离主窗独立摆放/多屏；自动入左侧插件栏（点击开/关、右键「弹出」） |
| 要给**消息卡片加按钮/信息**吗？ | 是 → `register_footer_action` / `register_footer_stat`（模板见 `templates-card-slots.md`） | 助手页脚 or 用户气泡操作行（role 分流） |
...
> **载体形态三选**（先定形态再谈容器）：浮动卡（主窗口内面板，默认首选）/
> 工作台页（右侧常驻页）/ 独立弹窗（`register_window`，可脱离主窗）。
> 判定：内容要**离开主窗**独立摆放 → 独立弹窗；否则用浮动卡/工作台页。
> 三者可复用同一 widget_class，但注册 API 不同。

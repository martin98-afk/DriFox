...
- **一、路径查找（dev / 用户环境兜底）** — L29–L112
  - 1.1 `_find_db()` — L37–L62
  - 1.2 `_get_db_connection()` — L63–L89
  - 1.3 路径层级计算说明 — L90–L112
- **二、N 天窗口查询模板** — L113–L216
  - 2.1 通用模板 — L119–L164
  - 2.2 多指标并行查询模板 — L165–L204
  - 2.3 为什么不用 `Pandas` / `Arrow`？ — L205–L216
- **三、字段 fallback 模式（新字段缺失时回退估算）** — L217–L317
  - 3.1 标准 fallback 模式 — L223–L257
  - 3.2 时间窗口内的 fallback（用于折线图每日统计） — L258–L305
  - 3.3 截断长度选择（防 OOM） — L306–L317
- **四、常见陷阱** — L318–L365
  - 4.1 时区问题（最隐蔽） — L331–L345
  - 4.2 数据库锁定问题 — L346–L365
- **五、完整读取函数骨架** — L366–L420
- **六、何时不用 SQLite 直读，改用主程序 API** — L421–L428
## 一、路径查找（dev / 用户环境兜底）

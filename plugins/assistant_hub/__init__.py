...
"助手（assistant / identity / AGENTS / pinned / today / longterm / dream / skills）"
完整数据模型，并把其能力复刻到 DriFox 的 PyQt5 体系内。
...
1. **助手 vs 智能体（双层抽象）**
   ...
   - **助手（assistant）**：本插件新引入的用户层概念。一个助手是
     "身份 + 人格 + 专属技能 + 私域记忆" 的封装，本质上把一个 assistant_id
     注入到一个**动态派生的 subagent** 来跑，原 build 智能体保持纯系统层不变。

2. **存储布局**
   每个助手存放在 ``<app_data_dir>/assistant_hub/<assistant_id>/``：
   - assistant.yaml               — 元数据（name / yuan / color / 主助手标记 / 技能白名单 / model 覆盖 / current_persona）
   - identity.md                  — 身份简介（openhanako persona-source 同款，回落模板）
   - AGENTS.md                    — 智能体提示词（assistant 的 system prompt）
...
3. **UI 入口**
   ...
   与 agent_trace 共用同一套 "标题栏 full-card" 模式：左侧助手列表，
   右侧多 Tab 编辑器（身份 / 提示词 / 头像 / 记忆 / 技能 / Dream）。

4. **运行时**
   选择某个助手后，``AssistantManager.set_active(assistant_id)`` 会同步
   AgentManager：注册一个动态 agent（name = ``assistant_<id>``，mode=subagent），
   把 identity.md + AGENTS.md 拼成 prompt、内存 exclude 全部 skill =

...
_STYLE = (
    "输出风格（读者是 LLM，非人类）：\n"
    "- 一条一碎片：`[标签] 对象: 事实`，标签取 [事实][决策][偏好][坑]，无合适则省略\n"
    "- 只写固定内容：已完成的进展、事实、决策、偏好/用户习惯、坑；\n"
    "  禁写待办、计划、未完成事项等实时性状态（它们天然过时，读了误导）\n"    "- 禁完整句、禁连接词、禁修饰语；主语可省。如「修 persona.py 打包丢 getpass」，\n"
...
def build_compile_today(turns: str, prev_today: str) -> list:
    ...
    return _msg(
        ...
        "- 草稿中已完成的待办改写成完成事实；未完成的待办直接丢弃，不带入输出\n"
...
def build_compile_daily(prev_today: str) -> list:
    return _msg(
        "把昨日全天「今日记忆」草稿蒸馏成 ≤3 行日记碎片，只留那天的重要进展、决定、踩坑。\n"
        + _STYLE
...
def build_dream_forget(facts_draft: str, longterm: str, today: str, daily: List[Dict[str, str]]) -> list:
    ...
    return _msg(
        ...
        "淘汰优先：待办与任务进度（无论是否完成）、一次性信息、可从外部重新获得的内容（公开文档/代码可查）、"
...
def build_consolidate(entries: List[Dict[str, str]]) -> list:
    ...
    return _msg(
        ...
        "- 分类纪律：分类宜少而稳定，优先沿用输入中已有的分类，相近分类可合并；确有必要才新建\n"
...
def build_reflect(identity_and_persona: str, memory_md: str, existing_experience: str) -> list:
    return _msg(
        ...
        "- 分类纪律：分类要少而稳定。必须优先从「已有经验库」里选一个最接近的分类，"

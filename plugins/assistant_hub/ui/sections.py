...
class ProfileSection(_Section):
    ...
    saveRequested = Signal(str, str, str)  # (name, user_addressing, utility_model_key)
    _DEBOUNCE_MS = 600
...
class AboutSection(_Section):
    ...
    personaChangeRequested = Signal(str)
    createPersonaRequested = Signal()
...
class ProjectSection(_Section):
    ...
    toggleNotes = Signal(bool)
    toggleContext = Signal(bool)
...
class PinnedSection(_Section):
    ...
    pinAddRequested = Signal(str)
    pinEdited = Signal(str, str)  # (pin_id, 新内容)
    pinDeleteRequested = Signal(str)
...
class MemorySection(_Section):
    ...
    toggleMemory = Signal(bool)
    toggleDreamAuto = Signal(bool)
    viewToday = Signal()
    dreamRun = Signal()
    dreamRestore = Signal()
    viewAll = Signal()
...
class ExperienceSection(_Section):
    ...
    toggleExperience = Signal(bool)
    viewCategory = Signal(str)
    deleteCategoryRequested = Signal(str)
    consolidateRequested = Signal()
    reflectRequested = Signal()

...
将图片放入主题文件夹即可：
```
~/.drifox6/plugins/user-custom/themes/{theme_id}/
├── {theme_id}.yaml
└── pet.png          # 你的像素宠物图片
```
...
2. 保存主题文件：放到用户主题目录，路径为 `~/.drifox6/plugins/user-custom/themes/{theme_id}/{theme_id}.yaml`
   - 先创建目录 `~/.drifox6/plugins/user-custom/themes/{theme_id}/`
   - 再写入 YAML 文件
   - 如果目录已存在，覆盖写入

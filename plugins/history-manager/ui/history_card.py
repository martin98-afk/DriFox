...
class _HistoryItemCard(QFrame):
    ...
    pinToggleRequested = Signal(int, bool)  # (index, 目标状态)
    moveToProjectRequested = Signal(int, str)  # (index, 目标项目)
...
class HistoryCard(QWidget):
    ...
    pinToggled = Signal(int, bool)  # 右键置顶/取消置顶
    moveToProjectRequested = Signal(int, str)  # 右键移动到项目
    dataChanged = Signal()  # 列表渲染完成（页面刷新项目下拉等联动）

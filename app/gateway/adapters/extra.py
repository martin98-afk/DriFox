...
# 注意：飞书适配器已移至 feishu.py，使用 lark_oapi SDK
    ...
    async def send(
        ...
        try:
            ...
            # 格式化内容 (飞书支持 Markdown)
            formatted = self._format_feishu_text(content)
            ...
            async with httpx.AsyncClient() as client:
                for i, chunk in enumerate(chunks):
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        if data.get("code") == 0:
                            message_ids.append(data.get("data", {}).get("message_id", ""))
    ...
    async def send_image(
        ...
        try:
            ...
            if upload_data.get("code") != 0:
                return SendResult(success=False, error=upload_data.get("msg", "Upload failed"))
            ...
            logger.error("[Feishu] Send image failed: %s", e)
            return SendResult(success=False, error=str(e))

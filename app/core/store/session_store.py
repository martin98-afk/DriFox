...
class SessionStore:
            ...
            try:
                ...
                self._db.create_table(
                    ...
                    [
                        ...
                        {"name": "token_count", "type": "INTEGER", "default": 0},
    ...
    def _migrate_add_token_count_column(self):
        """迁移：添加 token_count 列（如果不存在）"""
        if not self._db or not self._db.is_connected:
            return
        try:
            columns = self._db.get_table_info(self.TABLE_NAME)
            col_names = [c.get("name", "") for c in columns]
            if "token_count" not in col_names:
                logger.info("[SessionStore] 迁移：添加 token_count 列")
                self._db.execute_sql(f"ALTER TABLE {self.TABLE_NAME} ADD COLUMN token_count INTEGER DEFAULT 0")
                self._db.execute_sql(f"UPDATE {self.TABLE_NAME} SET token_count = 0 WHERE token_count IS NULL")
                logger.info("[SessionStore] token_count 列迁移完成")
        except Exception as e:
            logger.warning(f"[SessionStore] token_count 列迁移失败(可能已存在): {e}")

...
class AgentManager:
    ...
    def get_agent_system_prompt(
        ...
        global_contract = """
## Hook 消息说明
对话历史中可能包含以「🔌 **Hook 内部通知**」标记的独立消息块（用 `---` 分隔线包裹）。
这些是**系统内部钩子（Hook）产生的通知**，记录了系统运行时的内部事件（如会话启动、工具执行后处理等），
**不是用户的提问**，也**不是你之前输出的内容**。

当你看到此类消息时：
1. **不要**把它当作新用户输入来响应
2. **不要**中断当前的推理流程
3. 如果 Hook 消息中包含对当前任务有用的信息，可按需使用
4. 否则，**直接忽略它**，就像它不存在一样继续当前的思考

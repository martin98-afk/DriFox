...
def test_sync_chat_wrapper_width_applies_margins(qtbot):
    """_sync_chat_wrapper_width 必须真正应用 margins（回归：曾因死代码丢失 setContentsMargins）"""
    from PyQt5.QtWidgets import QWidget

    tm = TabManagerWindow.create_instance()
    qtbot.addWidget(tm)
    tm._chat_wrapper.resize(1800, 800)
    container = tm._global_top_container
    container._cards.pop("settings", None)

    # 对话区（index 0）→ pad = (1800-1000)/2 = 400
    tm._content_stack.setCurrentIndex(0)
    tm._sync_chat_wrapper_width()
    assert tm._chat_wrapper_layout.contentsMargins().left() == 400

    # 覆盖层 + 配置卡可见 → pad = (1800-1100)/2 = 350
    tm._content_stack.setCurrentIndex(1)
    card = QWidget()
    container._cards["settings"] = card
    card.setVisible(True)
    tm._sync_chat_wrapper_width()
    assert tm._chat_wrapper_layout.contentsMargins().left() == 350

    # 覆盖层 + 内容卡（diff_viewer）→ pad = 0 铺满
    card.setVisible(False)
    diff = QWidget()
    container._cards["diff_viewer"] = diff
    diff.setVisible(True)
    tm._sync_chat_wrapper_width()
    assert tm._chat_wrapper_layout.contentsMargins().left() == 0
